...
3. **Spec 업데이트와 Status 업데이트를 각각의 Reconcile로 분리한다.**
    ...
    **Spec**의 경우 **사용자가 작성(원하는 상태)**하고, **Status**의 경우 **Controller가 작성(현재 상태)**한다.
...
- 이러한 `client-go` 가 어떻게 `controller-runtime` 으로 추상화 되고, 이를 통해 Operator SDK가 어떻게 구성되는지 학습해보면 좋을 것 같다.
- Reconcile에서 구현해야 하는 영역이 아직 애매한 것 같다.
    - (”현재 상태 → 선언된 상태로 맞추는 로직”이라는 것이 감이 잡히지 않음)
- 학습한 설계원칙을 적용하지 않았을 때의 발생하는 문제도 실습해보면 좋을 것 같다.
