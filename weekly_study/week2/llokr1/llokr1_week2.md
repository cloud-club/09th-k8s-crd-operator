...
3. **Spec 업데이트와 Status 업데이트를 분리한다.**

    이는 현재 상태의 정보를 Indexer라는 로컬 캐시에서 가져오는 것을 고려해서 설계해야 하는 사항이다.

    `Update()`를 통해 오브젝트를 수정해도, 캐시에는 비동기로 갱신된다.

    따라서 `Update()` 직후에 수정 사항이 캐시에 반영되기 전에 `Get()`을 호출하면 이전 값이 반환될 수 있다.

#### Reconcile을 어떻게 구현 해야 할까?
