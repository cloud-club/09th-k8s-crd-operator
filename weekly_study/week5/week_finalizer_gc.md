...
예를 들어 `MyApp`이라는 Custom Resource를 만들었을 때 Operator가 다음과 같은 리소스를 생성한다고 가정한다.
...
이때 사용자가 `MyApp`을 삭제하면 Kubernetes 내부 리소스는 어느 정도 자동으로 정리될 수 있다.
...
대신 객체에 `deletionTimestamp`를 설정한다.
...
```text
이 리소스는 삭제 요청을 받았다.
하지만 finalizer가 남아 있으므로 아직 완전히 삭제되지 않는다.
...
```text
1. Custom Resource를 조회한다.
2. deletionTimestamp가 비어 있는지 확인한다.
3. 삭제 중이 아니면 finalizer가 있는지 확인한다.
4. finalizer가 없으면 추가한다.
5. 일반 Reconcile 로직을 수행한다.
6. 삭제 요청이 들어오면 deletionTimestamp가 생긴다.
7. finalizer가 있으면 외부 리소스 정리 작업을 수행한다.
8. 정리 성공 시 finalizer를 제거한다.
9. Kubernetes가 Custom Resource를 최종 삭제한다.
```
...
Reconcile 함수에서는 보통 `deletionTimestamp`를 기준으로 일반 상태와 삭제 상태를 구분한다.
...
그 이유는 finalizer가 `status`가 아니라 `metadata.finalizers`에 들어가기 때문이다.
...
사용자가 Custom Resource를 삭제하면 Kubernetes는 객체를 바로 지우지 않고 `deletionTimestamp`를 설정한다.
...
Garbage Collection은 Kubernetes가 `OwnerReference`를 기반으로 하위 리소스를 자동 삭제하는 기능이다.

예를 들어 `MyApp`이라는 Custom Resource가 있고, Operator가 Deployment와 Service를 만들었다고 하자.
...
이때 Deployment와 Service에 `OwnerReference`가 설정되어 있으면, `MyApp`이 삭제될 때 Kubernetes Garbage Collector가 하위 리소스도 함께 정리한다.
...
예를 들어 `MyApp`에 Finalizer가 있고, Deployment와 Service에는 OwnerReference가 있다고 하자.
...
Finalizer와 Garbage Collection은 모두 Kubernetes 삭제 흐름과 관련이 있다.

하지만 책임은 다르다.

```text
Garbage Collection
= OwnerReference를 보고 Kubernetes 내부 하위 리소스를 자동 삭제하는 기능이다.

Finalizer
= 리소스가 완전히 삭제되기 전에 Operator가 외부 리소스 정리 같은 작업을 끝내도록 삭제를 잠시 막는 기능이다.
```

쉽게 말하면 다음과 같다.

```text
Garbage Collection은 Kubernetes가 알아서 치우는 청소이다.
Finalizer는 Operator가 직접 마무리해야 하는 청소를 끝낼 때까지 문을 잠그는 장치이다.
```

따라서 Operator를 설계할 때는 다음 기준으로 나누어 생각하면 된다.

```text
Kubernetes 내부 리소스 정리
→ OwnerReference + Garbage Collection 사용

Kubernetes 외부 리소스 정리
→ Finalizer 사용

삭제 진행 상태 안내
→ Status Conditions + Event 사용
```
